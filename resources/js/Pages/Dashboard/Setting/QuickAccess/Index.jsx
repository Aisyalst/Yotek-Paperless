...
export default function Index({ quickAccesses = [] }) {
    const { auth } = usePage().props;
    const permissions = auth?.permissions || [];

    const hasReorderPermission = permissions.includes('quick-accesses.reorder');
    const hasEditPermission = permissions.includes('quick-accesses.edit');
    const hasDeletePermission = permissions.includes('quick-accesses.destroy');

    const [items, setItems] = useState(quickAccesses);
    const [draggedItem, setDraggedItem] = useState(null);

    useEffect(() => {
        setItems(quickAccesses);
    }, [quickAccesses]);
    ...
    const handleDrop = (e, targetItem) => {
        ...
        router.post('/quick-accesses/reorder', { ids }, { preserveScroll: true });
    };

    const handleDelete = (item) => {
        if (confirm(`Are you sure you want to delete "${item.title}"?`)) {
            router.delete(`/quick-accesses/${item.id}`);
        }
    };

    return (
        <DashboardLayout judulHalaman="Manajemen Akses Cepat">
            <Head title="Manajemen Akses Cepat" />

            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <div>
                    <h1 className="text-xl font-bold text-[#1a1a1a]">Daftar Akses Cepat</h1>
                    <p className="text-sm text-gray-600 mt-1">Kelola dan tarik (drag-and-drop) untuk mengurutkan akses cepat di beranda.</p>
                </div>

                <RedirectOutlineButton
                    text="Tambah Akses Cepat"
                    href="/quick-accesses/create"
                    routeName="quick-accesses.create"
                />
            </div>
            ...
            <div className="bg-[#ffffff] border border-gray-200 rounded-lg p-6 shadow-sm">
                <div className="space-y-3">
                    {items.length === 0 ? (
                        <p className="text-sm text-gray-500 py-4 text-center">Belum ada data akses cepat.</p>
                    ) : (
                        items.map((item) => (
                            <div
                                key={item.id}
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="p-2 bg-[#f8f8f8] border border-gray-200 rounded-md text-[#eaae36]">
                                        <DynamicIcon iconName={item.logo} />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-500 mt-0.5">
                                            URL/Route: <span className="font-mono text-gray-700">{item.url}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-2">
                                    {hasEditPermission && (
                                        <Link
                                            href={`/quick-accesses/${item.id}/edit`}
